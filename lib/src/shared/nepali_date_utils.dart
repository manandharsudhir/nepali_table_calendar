...
typedef NepaliDayBuilder = Widget? Function(
    BuildContext context, NepaliDateTime day);

/// Signature for a function that creates a widget for a given `day`.
/// Additionally, contains the currently focused day.
typedef NepaliFocusedDayBuilder = Widget? Function(
...
typedef NepaliTextFormatter = String Function(
    NepaliDateTime date, dynamic locale);

/// Gestures available for the calendar.
enum NepaliAvailableGestures { none, verticalSwipe, horizontalSwipe, all }

/// Formats that the calendar can display.
enum NepaliCalendarFormat { month, twoWeeks, week }

/// Days of the week that the calendar can start with.
enum NepaliStartingDayOfWeek {
  monday,
  tuesday,
  wednesday,
  thursday,
...
int getWeekdayNumber(NepaliStartingDayOfWeek weekday) {
  return NepaliStartingDayOfWeek.values.indexOf(weekday) + 1;
}
